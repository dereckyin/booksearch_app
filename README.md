...
輕量書架拍攝與檢貨展示 App，支援拍照上傳、離線佇列，以及檢貨清單 mock（後續可接正式後端）。
...
- 檢貨單 mock：`檢貨單` 按鈕進入列表，可點卡片預覽兩張本地示意圖（`temp_images`），之後再串後端。
- Manifest（可選）：如有 `MANIFEST_ENDPOINT`，上傳後回報尺寸/時間等資訊。

## 主要畫面
- `CaptureScreen`：相機預覽、拍照、離線佇列管理、重試上傳。
- `PickListScreen`：檢貨清單（目前 mock），點擊卡片進入 `PickItemPreviewScreen` 以本地圖示意。
